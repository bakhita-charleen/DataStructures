# --------------------- Lists ----------------------

# Q1. You have a list of daily bus fares collected: [150, 200, 150, 300, 250, 200, 150]. 
# Use append() to add today's fare of 180, then print the total number of fares recorded.

fares = [150, 200, 150, 300, 250, 200, 150]
print(fares)
fares.append(180)
print(fares)
print()
print(">" * 50)
print()
# Q2. A teacher has a list of students: ['Amos', 'Faith', 'Brian', 'Wanjiku']. A new student, 'Grace', 
# joins and should be second on the register. Use insert() to add her, then print the updated list.

students = ['Amos', 'Faith', 'Brian', 'Wanjiku']
print(students)
students.insert(1, 'Grace')
print(students)
print(">" * 50)
print()

# Q3. Given a list of scores [45, 78, 92, 33, 67, 88, 21], 
# use a list comprehension to create a new list containing only the scores that are 50 or above (a pass).

scores = [45, 78, 92, 33, 67, 88, 21]

passed = [score for score in scores if score >= 50]

print(passed)
print(">" * 50)
print()

# Q4. You have a list of product names: ['Sugar', 'Rice', 'Beans', 'Maize', 'Salt']. 
# Sort the list alphabetically using sort(), then use sorted() on the original unsorted order to sort it in reverse 
# - and print both results to show the difference between the two.

names = ['Sugar', 'Rice', 'Beans', 'Maize', 'Salt']
names.sort()
print(names)
print()

names = ['Sugar', 'Rice', 'Beans', 'Maize', 'Salt']
reverse_sort = sorted(names, reverse=True)
print(reverse_sort)
print()
print(">" * 50)
print()

# Q5. CHALLENGE: A shop has a stock list: ['Sugar', 'Rice', 'Beans', 'Maize'].
# Make a backup copy of it before removing 'Beans' (out of stock) from the live list. 
# Print both lists at the end to prove the backup was unaffected.

stock = ['Sugar', 'Rice', 'Beans', 'Maize']

#backup_stock = ['Sugar', 'Rice', 'Beans', 'Maize']
backup_stock = stock.copy()

stock.remove('Beans')
print(stock)
print(backup_stock)
print(">" * 50)
print()

# Tuples
# Q6. Create a tuple called location containing a town name and its coordinates:
#  ('Kericho', -0.3676, 35.2831). Print each element using indexing (not a loop).

location = ('Kericho', -0.3676, 35.2831)

print(location[0])
print(location[1])
print(location[2])
print(">" * 50)
print()

# Q7. Given the tuple dimensions = (12, 8), unpack it into two variables length and width in one line,
#  then print the area (length * width).

dimensions = (12, 8)
length, width = dimensions

print(f"Area is: {length * width}")
print(">" * 50)
print()

# Q8. Try to change the second item of the tuple fruits = ('Mango', 'Banana', 'Orange') to 'Pineapple' 
# using indexing (e.g. fruits[1] = 'Pineapple'). 
# Run it, note the error you get, and explain in one sentence why it happens.

fruits = ('Mango', 'Banana', 'Orange')
#fruits[1] = 'Pineapple'
print(fruits)
print()
# Reason: Error is because tuples are immutable/ cannot be modified

# Q9. You have a tuple of exam scores: (67, 89, 45, 92, 78). 
# Without converting it to a list, find and print the highest score, the lowest score, 
# and how many scores there are.

scores = (67, 89, 45, 92, 78)
highest = max(scores)
lowest = min(scores)
score_count = len(scores)

print(f"Highest score is: {highest}")
print(f"Lowest score is: {lowest}")
print(f"Score count is: {score_count}")
print(">" * 50)
print()

# Q10. CHALLENGE: A list contains three tuples, each representing a student's name and score: 
# [('Amos', 78), ('Faith', 92), ('Brian', 65)]. 
# Loop through the list and print each student's name and score in the format 'Amos scored 78'. 
# (Hint: unpack each tuple directly in the for line.)

students = {
    ('Amos', 78), 
    ('Faith', 92), 
    ('Brian', 65)
}
for name, score in students:
    print(f"{name} scored {score}")

print(">" * 50)
print()

# ------------------------------ Dictionaries--------------------------------
# Q11. Create a dictionary called student with keys name, age, and class, holding your own made-up values.
#  Print the student's name using key access.

student = {'name':'Kylie', 'age': '30', 'class': '11'}
print(student['name'])
print(student.get('name'))
print(">" * 50)
print()

# Q12. Given prices = {'Sugar': 150, 'Rice': 120, 'Beans': 180}, add a new item 'Maize': 90 to the dictionary, 
# then update 'Sugar' to 160. Print the final dictionary.

prices = {'Sugar': 150, 'Rice': 120, 'Beans': 180}
print(prices)

prices['Maize'] = 90
prices['Sugar'] = 160
print(prices)
print(">" * 50)
print()

# Q13. Loop through this dictionary and print each item in the format 'Sugar costs Ksh 150': 
# prices = {'Sugar': 150, 'Rice': 120, 'Beans': 180}. (Hint: use .items())

prices = {'Sugar': 150, 'Rice': 120, 'Beans': 180}

for item, price in prices.items():
    print(f"{item} costs Ksh {price}")
print(">" * 50)
print()

# Q14. Given stock = {'Sugar': 40, 'Rice': 0, 'Beans': 15, 'Maize': 0}, 
# use a loop to print only the products that are out of stock (value is 0).

stocks = {'Sugar': 40, 'Rice': 0, 'Beans': 15, 'Maize': 0}

for item, quantity in stocks.items():
    if quantity == 0:
        print(item)
print(">" * 50)
print()

# Q15. CHALLENGE: You have sales data for a week: 
# sales = {'Mon': 4200, 'Tue': 3800, 'Wed': 5100, 'Thu': 4700, 'Fri': 6200}. 
# Find and print the day with the highest sales and its amount, without using max() directly on the dictionary -
#  loop through it and track the highest manually.

sales = {'Mon': 4200, 'Tue': 3800, 'Wed': 5100, 'Thu': 4700, 'Fri': 6200}

highest_sale = 0
highest_day = ""

for day, sale in sales.items():
    if sale > highest_sale:
        highest_sale = sale
        highest_day = day

print(f"{highest_day}:{highest_sale}")
print(">" * 50)
print()

